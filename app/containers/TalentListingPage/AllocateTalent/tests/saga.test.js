...
import hireTalent, { submitAllocate } from '../saga';
...
describe('Testing submitAllocate form', () => {
  test('Request With Status One', async () => {
    request.mockImplementation(() => Promise.resolve({ status: 1 }));
    await recordSaga(submitAllocate);
...
describe('Testing setStateLicenseInformations', () => {
  test('setStateLicenseInformations', () => {
    ...
    expect(gen.next().value).toEqual(takeLatest(SUBMIT_HIRE, submitAllocate));
  });
});
