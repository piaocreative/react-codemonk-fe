...
import addTimesheetForm, { submitTimesheet } from '../saga';
...
describe('Testing submitTimesheet form', () => {
  test('Request With Status One', async () => {
    request.mockImplementation(() => Promise.resolve({ status: 1 }));
    await recordSaga(submitTimesheet);
...
describe('Testing setStateLicenseInformations', () => {
  test('setStateLicenseInformations', () => {
    const gen = addTimesheetForm();
    expect(gen.next().value).toEqual(takeLatest(SUBMIT_ADD_TIMESHEET, submitTimesheet));
  });
});
