...
import { filterIcon, yearsOfExperienceArray, roles, skillsData, showFilterArray } from 'containers/App/constants';
...
import { isNoFilterApplied } from './utils';
import messages from './messages';

export class ClientBriefFilters extends Component {
  constructor(props) {
    super(props);
    ...
    const updatedFilter = isEmpty(storedFilter)
      ...
      : storedFilter;
    dispatch(change(key, 'briefSkills', get(updatedFilter, 'skillsArray', [])));
  ...
  handleCheckboxFilterChange = (name, checked, type) => {
    ...
    if (type === 'showFiltArray' && newFilter.length > 0) {
      newFilter = newFilter.slice(-1);
      if (name === 'all') {
        newFilter = ['all'];
      } else {
        const exp = newFilter.filter(val => val === name);
        newFilter = exp;
      }
    }
  ...
  handleSkillsChange = e => {
    ...
    this.setState({ filters: newFilters, totalFilters });
    dispatch(change(key, 'briefSkills', skillsVal));
  };
  ...
  filterChanged = type => {
    ...
    if (type === 'update') {
      ...
      dispatch(change(key, 'briefSkills', []));
    }
    ...
    StorageService.set('clientBriefFilter', JSON.stringify(updatedFilter));
      ...
      <React.Fragment>
        ...
        >
                ...
                {teamPreferenceArray
                  ...
                  .map(item => (
                    <Field
                      ...
                      onChange={e => this.handleCheckboxFilterChange(e.target.name, e.target.checked, 'teamPrefArray')}
                    />
                  ))}
              </FormGroup>
            </Col>
                ...
                {assignmentArray
                  ...
                  .map(item => (
                    <Field
                      ...
                      onChange={e => this.handleCheckboxFilterChange(e.target.name, e.target.checked, 'assignmentsArray')}
                    />
                  ))}
              </FormGroup>
            </Col>
              ...
              <FormGroup className="input-sm">
                {projectPreference.map(item => (
                  <Field
                    ...
                    onChange={e => this.handleCheckboxFilterChange(e.target.name, e.target.checked, 'workPrefArray')}
                  />
                ))}
              </FormGroup>
            </Col>
              ...
              <FormGroup className="input-sm">
                {yearsOfExperienceArray.map(item => (
                  <Field
                    ...
                    onChange={e => this.handleCheckboxFilterChange(e.target.name, e.target.checked, 'expertiseArray')}
                  />
                ))}
              </FormGroup>
            </Col>
          ...
          <Row>
              ...
              <FormGroup className="input-sm">
                {roles.map(item => (
                  <Field
                    ...
                    onChange={e => this.handleCheckboxFilterChange(e.target.name, e.target.checked, 'roleArray')}
                  />
                ))}
              </FormGroup>
            </Col>
            ...
            <Col md="6">
              <H4 className="input-sm mt-0">
                <FormattedMessage {...containerMessage.subShowFilter} />
              </H4>
              <FormGroup className="input-sm">
                {showFilterArray.map(item => (
                  <Field
                    key={item.value}
                    name={item.value}
                    type="checkbox"
                    component={renderCheckBox}
                    label={item.label}
                    checked={get(filters, 'showFiltArray', []).includes(item.value)}
                    onChange={e => this.handleCheckboxFilterChange(e.target.name, e.target.checked, 'showFiltArray')}
                  />
                ))}
              </FormGroup>
            </Col>
...
ClientBriefFilters.defaultProps = defaultProps;
ClientBriefFilters.propTypes = propTypes;
